...
describe("parabun:gpu scaffold", () => {
  ...
  it("winsForSize returns false for ops no backend beats simd on yet", async () => {
    ...
    // backend says "don't use me", so the caller falls through to @lyku/para-simd.
  ...
  it("matVec f32 over the GPU threshold matches @lyku/para-simd bit-for-bit", async () => {
    ...
    // keeps the same interface. Either way, output must match @lyku/para-simd
    ...
    // Tolerance: per-row FMA on Metal is identical to @lyku/para-simd's scalar
    ...
    // @lyku/para-simd, on faster Metal devices it already wins. This test pins
  ...
  it("dot matches @lyku/para-simd on cpu fallback", async () => {
    const { stdout, exitCode } = await runFixture(
      "parabun-gpu-dot-f32",
      `
        import gpu from "parabun:gpu";
  ...
  it("matVec matches @lyku/para-simd on cpu fallback", async () => {
    const { stdout, exitCode } = await runFixture(
  ...
  it("matVec over alloc'd inputs matches matVec over plain inputs", async () => {
    ...
    //
    // On Linux (cpu fallback) both paths collapse to the same @lyku/para-simd call,
  ...
  it("matVec on a released handle throws; release is idempotent", async () => {
    ...
    expect(r).toEqual({
      ...
      matVecThrew: true,
      // Matches the existing "matVec matches @lyku/para-simd on cpu fallback" test.
      out: [10, 26, 42],
    });
    expect(exitCode).toBe(0);
  });
  ...
  it("matVec over a held matrix is bit-identical to matVec over the same array", async () => {
    ...
    // both paths collapse to the same @lyku/para-simd call — still must match.
    const { stdout, exitCode } = await runFixture(
...
describe("parabun:gpu per-host calibration (CUDA simdMap crossover)", () => {
  // `calibrate()` sweeps the real PTX kernel vs @lyku/para-simd and persists a
