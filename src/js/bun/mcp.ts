...
const DEFAULT_CLIENT_INFO = { name: "bun:mcp", version: "0.1.0" } as const;
...
class MCPConnection {
  ...
  async #request<T = any>(method: string, params?: unknown): Promise<T> {
    if (this.#closed) throw new Error("bun:mcp: connection is closed");
  ...
  #onTransportClose(err?: Error): void {
    if (this.#closed) return;
    this.#closed = true;
    const cause = err ?? new Error("bun:mcp: transport closed");
  ...
  async call(name: string, args: Record<string, unknown> = {}): Promise<ToolCallResult> {
    if (typeof name !== "string" || !name) {
      throw new TypeError("bun:mcp: tool name must be a non-empty string");
    }
  ...
  async close(): Promise<void> {
    ...
    for (const p of this.#pending.values()) {
      p.reject(new Error("bun:mcp: connection closed"));
    }
    this.#pending.clear();
    try {
...
function makeStdioTransport(command: string, opts: StdioConnectOpts): Transport {
  ...
  proc.on("exit", code => {
    if (closed) return;
    closed = true;
    onClose?.(code === 0 ? undefined : new Error(`bun:mcp: stdio server exited with code ${code}`));
  });
  proc.on("error", err => {
    if (closed) return;
    closed = true;
  ...
  return {
    send(msg) {
      if (closed) throw new Error("bun:mcp: stdio transport is closed");
...
function makeWsTransport(url: string): Promise<Transport> {
  return new Promise((resolve, reject) => {
    ...
    ws.addEventListener("open", () => {
      ...
      resolve({
        send(msg) {
          if (closed) throw new Error("bun:mcp: ws transport is closed");
    ...
    ws.addEventListener("error", () => {
      ...
      reject(new Error(`bun:mcp: WebSocket error connecting to ${url}`));
    });
    ...
    ws.addEventListener("close", e => {
      ...
      closed = true;
      if (!opened) {
        reject(new Error(`bun:mcp: WebSocket closed before open (code=${e.code})`));
        return;
      }
      onClose?.(e.code === 1000 ? undefined : new Error(`bun:mcp: WebSocket closed (code=${e.code})`));
    });
  });
}
...
async function connect(
  ...
  if (transport === "stdio") {
    ...
    throw new Error(`bun:mcp: unknown transport "${transport}". Use "stdio" or "ws".`);
  }
