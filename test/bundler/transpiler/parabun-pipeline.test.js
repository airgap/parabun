...
describe("@lyku/para-pipeline", () => {
  it("map + collect", async () => {
    const { stdout, exitCode } = await runFixture(
      "parabun-pipe-map",
      `
        import { map, collect } from "@lyku/para-pipeline";
  ...
  it("take is lazy (does not pull past the limit)", async () => {
    const { stdout, exitCode } = await runFixture(
      "parabun-pipe-lazy",
      `
        import { take, collect } from "@lyku/para-pipeline";
        let pulls = 0;
        function* source() {
  ...
  it("drop + takeWhile", async () => {
    const { stdout, exitCode } = await runFixture(
      ...
      `
        import { drop, takeWhile, collect } from "@lyku/para-pipeline";
  ...
  it("flatMap", async () => {
    const { stdout, exitCode } = await runFixture(
      ...
      `
        import { flatMap, collect } from "@lyku/para-pipeline";
  ...
  it("chunk", async () => {
    const { stdout, exitCode } = await runFixture(
      "parabun-pipe-chunk",
      `
        import { chunk, collect } from "@lyku/para-pipeline";
  ...
  it("reduce", async () => {
    const { stdout, exitCode } = await runFixture(
      "parabun-pipe-reduce",
      `
        import { reduce } from "@lyku/para-pipeline";
  ...
  it("count", async () => {
    const { stdout, exitCode } = await runFixture(
      "parabun-pipe-count",
      `
        import { filter, count } from "@lyku/para-pipeline";
  ...
  it("range + take + collect", async () => {
    const { stdout, exitCode } = await runFixture(
      "parabun-pipe-range",
      `
        import { range, take, collect } from "@lyku/para-pipeline";
  ...
  it("pipe() call-form equivalent", async () => {
    const { stdout, exitCode } = await runFixture(
      ...
      `
        import { pipe, map, filter, collect } from "@lyku/para-pipeline";
  ...
  it("fusion: Float64Array |> map(affine) |> map(affine) |> toFloat64Array", async () => {
    const { stdout, exitCode } = await runFixture(
      ...
      `
        import { map, toFloat64Array } from "@lyku/para-pipeline";
  ...
  it("fusion: chain + filter falls back correctly", async () => {
    const { stdout, exitCode } = await runFixture(
      ...
      `
        import { map, filter, collect } from "@lyku/para-pipeline";
        pure function double(x) { return x * 2; }
  ...
  it("fusion: large f32 affine chain matches @lyku/para-simd (GPU tier)", async () => {
    ...
    const { stdout, exitCode } = await runFixture(
      ...
      `
        import { map, toFloat32Array } from "@lyku/para-pipeline";
        ...
        // Reference: same chain through @lyku/para-simd directly.
  ...
  it("fusion: empty chain on typed array is identity", async () => {
    const { stdout, exitCode } = await runFixture(
      ...
      `
        import { collect, sum } from "@lyku/para-pipeline";
