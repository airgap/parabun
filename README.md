...
| shipped     | `bun:gpio`            | Linux GPIO uAPI v2 — `chips()` / `open()` with single-line `read()` / `write()` / `toggle()` / `edges()` async iterator + reactive `value` signal, plus atomic multi-line `chip.bank(offsets, opts)` for synchronous multi-pin transitions in one ioctl. Same surface across RPi 4, RPi 5 (pinctrl-rp1), Jetson, any Linux SBC. |
