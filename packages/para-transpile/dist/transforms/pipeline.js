...
export function transformPipeline(src) {
...
function collapsePipelinesInCode(code) {
    let out = "";
    let i = 0;
    while (i < code.length) {
 ...
 */
function findNextTopLevelPipe(code, from) {
    let depth = 0;
 ...
 */
function scanLhsStart(code, pipePos) {
    let depth = 0;
    let i = pipePos - 1;
 ...
 */
function scanRhsEnd(code, startPos) {
    let depth = 0;
    let i = startPos;
    while (i < code.length) {
        const c = code[i];
...
function collapsePipelineChain(expr) {
...
function hasTopLevelPlaceholder(args) {
    let depth = 0;
...
function substitutePlaceholder(args, replacement) {
    let out = "";
    let depth = 0;
    for (let i = 0; i < args.length; i++) {
        const c = args[i];
