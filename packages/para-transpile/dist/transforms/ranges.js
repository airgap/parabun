...
export function transformRanges(src) {
    return rewriteCodeRegions(src, code => {
        ...
        out = out.replace(new RegExp(String.raw `(${operand})\.\.=(${operand})`, "g"), (_m, a, b) => `__parabunRangeInclusive(${a}, ${b})`);
        // Exclusive form. Same operand shape; the negative lookahead on `.` /
        // `=` prevents matching `..=` (already handled) or `...` (spread).
        out = out.replace(new RegExp(String.raw `(${operand})\.\.(?![.=])(${operand})`, "g"), (_m, a, b) => `__parabunRange(${a}, ${b})`);
        return out;
    });
}
