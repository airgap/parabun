/**
 * libjpeg-turbo — JPEG decode/encode with SIMD acceleration. Backs both
 * upstream `Bun.Image` (via the high-level TurboJPEG API in turbojpeg.h)
 * and Para's `parabun:image` codec (via the low-level libjpeg C API
 * `jpeg_create_decompress` etc); both surfaces compile out of the same
 * direct build below.
 *
 ...
 * (turbojpeg.c gates them on `#ifdef NO_PRECISION_EXT`); both Bun.Image and
 * parabun:image only deal in 8-bit RGB(A).
 */
...
export const libjpegTurbo: Dependency = {
  ...
  provides: cfg => ({
    libs: [],
    // Public headers are <turbojpeg.h> + <jpeglib.h> (low-level API used by
    // parabun:image) in src/; jconfig.h is generated into the build dir.
