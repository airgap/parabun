...
3. **Run the scoring across workers with `@lyku/para-parallel`.** The embedding
...
features (`pure function` for worker fns, `@lyku/para-parallel` for the pool)
...
- `parabun-store.pjs` — drop-in `extends VectorStore` with the same method
  signatures. Internals use `@lyku/para-parallel.pmap` + SAB.
