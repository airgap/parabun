...
// WeakSet of nodes (or sub-arrows of tracked calls) whose body should
// receive bare-read rewriting.
type TrackedSet = WeakSet<t.Node>;
...
export function transformBareRead(src: string): string {
  ...
  const trackedArrows: TrackedSet = new WeakSet();
  let dirty = false;
    ...
    CallExpression(path) {
      if (isTrackedCallee(path.node.callee)) {
        for (const arg of path.node.arguments) {
          if (t.isArrowFunctionExpression(arg)) trackedArrows.add(arg);
        }
      }
    },
  ...
  traverse(ast, {
    Identifier(path) {
      if (!isSignalRef(path)) return;
      if (!isInTrackedContext(path, trackedArrows)) return;
...
function isTrackedCallee(callee: t.Node): boolean {
  return (
    isSignalsRequireMember(callee, "effect") ||
...
function isInTrackedContext(path: any, trackedArrows: TrackedSet): boolean {
  let p = path.parentPath;
  while (p) {
    if (t.isArrowFunctionExpression(p.node) && trackedArrows.has(p.node)) return true;
    p = p.parentPath;
  }
  return false;
}

function isSignalRef(path: any): boolean {
  const binding = path.scope.getBinding(path.node.name);
  return !!binding && !!(binding as any)[SIGNAL_FLAG];
}
 ...
 */
function isReadReference(path: any): boolean {
