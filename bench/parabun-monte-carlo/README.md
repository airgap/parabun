...
single-threaded vs `bun:parallel.pmap` across 8 workers. No SIMD — this
...
What makes the `bun:parallel` primitive feel light here:
...
- `variant-parabun.pjs` — same math, split across 8 workers via
  `bun:parallel.pmap` with independent PRNG streams.
