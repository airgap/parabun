...
function emitCargo(n: Ninja, cfg: Config, name: string, spec: CargoBuild, input: EmitCargoInput): { libs: string[] } {
  ...
  // objects (EM: 183 mismatch). Para-specific path; upstream's
  // cfg.crossTarget block below handles Android + FreeBSD.
  ...
  // Cross-compile (Android / FreeBSD via cfg.crossTarget): cargo's default
  // `cc` linker can't handle the foreign ELF objects. Use our clang as the
  // linker driver and pass --target/--sysroot through, same as the C/C++
  // deps do via globalFlags. The cdylib output also wants -lunwind, which
  // lives in the NDK's bundled clang resource dir (not the sysroot), so we
  // add that -L too. Mutually exclusive with the linux-arm64 branch above
  // — that path doesn't set cfg.crossTarget.
