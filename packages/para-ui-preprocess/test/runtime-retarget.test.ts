...
// `svelte` runtime and the F0 fork `@para/ui` runtime. Drives the script-tag
...
describe("runtime retarget", () => {
  test("default runtime is @para/ui", async () => {
    const out = await callScript(PUI_SCRIPT);
    expect(out?.code).toContain('from "@para/ui"');
    expect(out?.code).not.toContain('from "svelte"');
  });

  test('explicit runtime: "@para/ui" emits @para/ui import', async () => {
    const out = await callScript(PUI_SCRIPT, { runtime: "@para/ui" });
    expect(out?.code).toContain('from "@para/ui"');
  });
  ...
  test('explicit runtime: "svelte" emits legacy svelte import', async () => {
    ...
    expect(out?.code).not.toContain('from "@para/ui"');
  });

  test("dedup honors either runtime spelling on hand-authored imports", async () => {
    // If the script already imports onDestroy from @para/ui, default-runtime
    // emission must NOT add a duplicate `from "svelte"` import for onDestroy.
    const withExisting = `
import { onDestroy } from "@para/ui";
${PUI_SCRIPT}
`;
    const out = await callScript(withExisting, { runtime: "svelte" });
    // We asked for svelte runtime but onDestroy already came from @para/ui —
