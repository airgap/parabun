// Llama-3 BPE tokenizer for parabun:llm.
//
      ...
      for (const piece of pieces) {
        ...
        if (id === undefined) {
          ...
          for (const ch of piece) {
            ...
            if (charId === undefined) {
              throw new Error(`parabun:llm: tokenizer: no vocab entry for piece "${piece}" or char "${ch}"`);
            }
            out.push(charId);
          }
        } else {
...
function fromGGUF(gguf: { metadata: Map<string, unknown> }): LlamaTokenizer {
  ...
  if (model !== "gpt2") {
    throw new Error(`parabun:llm: tokenizer model "${model}" not supported (want "gpt2")`);
  }
  ...
  else throw new Error(`parabun:llm: tokenizer pre "${preRaw}" not supported (want "llama-bpe" or "qwen2")`);
  ...
  if (!Array.isArray(merges)) throw new Error("parabun:llm: missing tokenizer.ggml.merges");
  if (typeof eosRaw !== "number") throw new Error("parabun:llm: missing tokenizer.ggml.eos_token_id");
