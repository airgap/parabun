...
function transformEffectBlocks(src: string): string {
  return rewriteKeywordBlocks(src, "effect", body => `require("@lyku/para-signals").effect(() => {${body}})`);
}
...
function peekPairedWhenNot(src: string, from: number): { body: string; end: number } | null {
  // Skip whitespace.
  let i = from;
  while (i < src.length && /\s/.test(src[i]!)) i++;
  // Must be `when` (followed by space).
  if (!src.startsWith("when", i) || !/\s/.test(src[i + 4] ?? "")) return null;
  i += 4;
  while (i < src.length && /\s/.test(src[i]!)) i++;
  // Must be `not`.
  if (!src.startsWith("not", i) || !/[\s{]/.test(src[i + 3] ?? "")) return null;
  i += 3;
  while (i < src.length && /\s/.test(src[i]!)) i++;
