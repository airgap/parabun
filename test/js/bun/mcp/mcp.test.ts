...
// @lyku/para-mcp client coverage (LYK-733).
//
...
describe("@lyku/para-mcp", () => {
  describe("stdio transport", () => {
    test("connect, list tools, call, close", async () => {
      const mcp = (await import("@lyku/para-mcp")).default;
  ...
  describe("ws transport", () => {
    ...
    test("connect rejects when ws server is unreachable", async () => {
      ...
      await expect(mcp.connect("ws", "ws://127.0.0.1:1")).rejects.toThrow("@lyku/para-mcp");
    }, 30000);
  });
