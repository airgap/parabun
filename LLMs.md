...
Statement-level cell sugar. `signal NAME = EXPR` desugars to `const NAME = require("@lyku/para-signals").signal(EXPR)`, and the declared name is signal-bound so bare reads of `NAME` lower to `NAME.get()` and bare assignments (`NAME = X`, `NAME += X`, `NAME++` …) lower to the matching `.set()` / `.update()` call. `signal` always implies `const` — there's no `signal let` / `signal var`.
...
**Auto-derive (signal → derived).** If the RHS reads another in-scope signal name, `signal NAME = EXPR` is silently promoted to `const NAME = require("@lyku/para-signals").derived(() => EXPR)` so `NAME` re-derives whenever its deps change:
...
`derived` keeps its identifier reading when not immediately followed by an identifier — `import { derived } from "@lyku/para-signals"; const x = derived(() => …);` works unchanged.

### `~>` (reactive binding operator)

Desugars `A ~> B` to `require("@lyku/para-signals").effect(() => { B = A; })` — an `effect` that evaluates `A` in a tracked context and re-assigns `B` whenever any signal read by `A` changes. Precedence: assign level (lower than `|>`, so `a |> f ~> sink` parses as `(a |> f) ~> sink`).
...
Desugars `A -> fn` to `require("@lyku/para-signals").effect(() => { fn(A); })` — the call-sink complement to `~>`. When `A` reads signals, any change re-runs the body and re-calls `fn` with the latest value. Same precedence as `~>` (assign level), same disposer return shape, same optional `when COND` guard.
...
**Disambiguation.** `parallel` is a contextual keyword — only the two adjacent forms (`parallel {` and `parallel let|const`) trigger the rewrite. Anywhere else (`parallel(x)`, `parallel.foo`, `parallel = 1`, `import { parallel } from "@lyku/para-parallel"`) it stays a normal identifier.
...
`.pui` is a multi-year buildout tracked under [LYK-829](https://linear.app/lyku/issue/LYK-829). Phase 0 (file format + minimum LSP) is what's currently usable. Later phases add script-keyword sugar (`store`/`derived`/`effect`/`mount`/`source`/`async signal`/`prop`/`emit`/`using`), parabun operators in template `{}` expressions, `{#match}`/`{#try}`, `pure component`, server/client split, and more — see umbrella for details.
...
### `@lyku/para-parallel` — `pmap`, `preduce`
...
```
import { pmap, preduce } from "@lyku/para-parallel";
...
- if all affine, collapse the chain to a single `(K, C)` and dispatch to
  `@lyku/para-simd` (`mulScalar`/`addScalar`) — one pass over the array,
...
`@lyku/para-parallel` for data parallelism. Consecutive `map` stages are composed
...
```
import { map, filter, reduce, pipeParallel } from "@lyku/para-pipeline";
...
### `@lyku/para-simd` — vector primitives for typed arrays
...
```
import { mulScalar, add, dot, simdMap } from "@lyku/para-simd";
...
a typed array backed by the `@lyku/para-simd` WASM instance's linear memory.
...
```js
import { alloc, mulScalar } from "@lyku/para-simd";
...
| op                  | array | `.map`/`.reduce` | tight loop | `@lyku/para-simd` |
|---------------------|:-----:|-----------------:|-----------:|-----------:|
...
Compute-only GPU surface (no graphics) that mirrors a subset of `@lyku/para-simd`.
Probes a backend chain — Metal on darwin, CUDA on Linux/Windows, always
falling back to a CPU path that just forwards to `@lyku/para-simd`. All callers
...
commit-for-lifetime model as `@lyku/para-simd.alloc`). On CPU/CUDA, `alloc`
...
- **Wins threshold** (`winsForSize`) — above this size, *callers* should
  route to `parabun:gpu` rather than `@lyku/para-simd`. `@lyku/para-pipeline`'s fusion tier
...
- **CUDA** (`src/js/bun/gpu/cuda.ts`) — PTX kernels loaded via the
  ...
  `@lyku/para-simd` at every measured size because per-call HtoD dominates.
  ...
  `@lyku/para-simd` on held 128 MB dot) on an RTX 4070 Ti.
...
- **CPU** (`src/js/bun/gpu/cpu.ts`) — every op forwards to `@lyku/para-simd`.

Pipeline integration: when a fused affine chain on a `Float32Array` is
large enough (`winsForSize("simdMap", n, 4)`), `@lyku/para-pipeline`'s
...
- **Tier 1** (codecs + capture + inference + peripheral I/O):
  ...
  entries), `@lyku/para-csv`, `parabun:llm` (GGUF Llama/Qwen2 + BERT
  ...
  for tool dispatch), `@lyku/para-rtp` (with `JitterBuffer.pendingSignal` /
  `lossCountSignal` / `lossRateSignal`), `@lyku/para-mcp` (Model Context
...
- **Tier 2** (applications): `parabun:speech` (`listen` / `transcribe` /
  ...
  `parabun:audio` + `parabun:speech` + `parabun:llm` / `@lyku/para-mcp`, ships
  ...
  stubbed), `@lyku/para-arrow` (in-memory tables + computes + IPC streaming
...
Parse-time syntax (`pure`, `memo`, `|>`, `..!`, `..&`, `..>`, `..` / `..=` ranges, `Nd` decimal literals, `defer` / `defer await`, `throw` as expression) compiles to plain JS and runs in a browser unchanged. The runtime-backed features do NOT — `arena { body }` imports `@lyku/para-arena`, `signal` / `effect` / `~>` import `@lyku/para-signals`, and `memo` / range / decimal literals import `bun:wrap`. Bundlers targeting `browser` can't resolve these specifiers by default.

The cross-runtime Lib modules (`signals`, `parallel`, `pipeline`, `arena`, `simd`, `csv`, `arrow`, `rtp`, `mcp`) ship as individual `@lyku/para-*` npm packages. Applications targeting non-Parabun hosts alias the `para:*` specifiers onto them — typically with one regex rule:
...
The few specifiers that aren't `@lyku/para-*` packages yet (`bun:wrap` for the wrap-macro runtime, `parabun:gpu` and `parabun:llm` for the WebGPU / Wasm shims) come from [`packages/parabun-browser-shims`](packages/parabun-browser-shims):
...
- **`@lyku/para-arena`** — `Pool` (typed-array free list) + no-op `scope()` (browsers don't expose GC control; inline body is semantically correct).
- **`@lyku/para-signals`** — full implementation of `signal` / `derived` / `effect` / `batch` / `untrack`, plus `fromAsync(asyncIterable)` and `fromInterval(fn, periodMs)` — the latter the canonical "periodic source → reactive signal" helper for IoT sensor reads.
- **`@lyku/para-parallel`** — Web Worker pool. `pmap` / `preduce` dispatch across workers; transparent sequential fallback under CSP or non-browser hosts.
- **`@lyku/para-simd`** — WebAssembly v128 kernels with scalar JS fallback. Typical 3-20× speedup over scalar.
- **`@lyku/para-pipeline`**, **`@lyku/para-csv`**, **`@lyku/para-arrow`**, **`@lyku/para-rtp`**, **`@lyku/para-mcp`** — full implementations.
...
- **`parabun:gpu`** (parabun-browser-shims) — CPU + WebGPU compute shaders for `matVecAsync` / `matmulAsync` / `dotAsync`. CPU fallback via `@lyku/para-simd`.
...
- **Variant C** (`variant-c.pjs`): columnar extraction into
  `Float64Array`, `@lyku/para-simd` `sum`/`dot` for mean/stddev/weighted-dot,
...
`@lyku/para-simd.topK` replaces the idiomatic `map → sort → slice` that was
...
- **Auto-accel dispatch, Tier 4 (implicit cross-call residency)** — the
  ...
  `matmul`, `simdMap`). On an RTX 4070 Ti the held path beats `@lyku/para-simd`
