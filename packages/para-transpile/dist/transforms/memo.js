...
export function transformMemo(src) {
    if (!src.includes("memo"))
        return src;
...
function tryParseMemo(src, memoPos) {
    ...
    const isDeclStart = prev < 0 || prevChar === ";" || prevChar === "{" || prevChar === "}" || prevChar === "\n" || isExported;
    ...
    if (nameMatch && isDeclStart) {
        ...
        while (afterParen < src.length && /\s/.test(src[afterParen]))
            afterParen++;
        let typeAnnot = "";
        if (src[afterParen] === ":") {
    ...
    if (src[i] === "(" || src[i] === "<") {
        ...
        if (src[afterParen] === ":") {
            ...
            while (afterParen < src.length && !(src[afterParen] === "=" && src[afterParen + 1] === ">"))
                afterParen++;
        }
        if (src.slice(afterParen, afterParen + 2) !== "=>")
            return null;
...
function findArrowBodyEnd(src, from) {
    let i = from;
