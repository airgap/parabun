...
Microbenchmark: `@lyku/para-simd.matVec` vs `parabun:gpu.matVec` across a grid of
...
`parabun:gpu.matVec` and `@lyku/para-simd.matVec` resolve to the same tight SIMD
...
- The matrix is filled with a deterministic LCG, so every run sees the
  ...
  matches `@lyku/para-simd` closely enough that this is usually exact.
- `winsForSize("matVec", …)` returns `false` for every size right now.
  ...
  beat `@lyku/para-simd`, drop `MIN_MATVEC_WINS_ELEMS` back down to a concrete
  number and these two constants collapse into one.
- `parabun:gpu.matVec` on hosts with no real GPU backend (cpu backend) just
  forwards to `@lyku/para-simd`, so there's no harm in calling it regardless.
