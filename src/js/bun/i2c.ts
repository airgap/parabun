...
// fd is freed at GC time rather than leaking. Same pattern as bun:gpio.
...
class BusImpl implements Bus {
  ...
  assertOpen(): void {
    if (this.#closed) throw new Error("bun:i2c: bus is closed");
  }
  ...
  device(addr: number): Device {
    ...
    if (typeof addr !== "number" || !Number.isInteger(addr) || addr < 0 || addr > 0x7f) {
      throw new RangeError(`bun:i2c: addr must be a 7-bit integer in [0x00, 0x7f], got ${addr}`);
    }
...
function open(path: string): Bus {
  if (typeof path !== "string" || path.length === 0) {
    throw new TypeError("bun:i2c.open: path must be a non-empty string");
  }
