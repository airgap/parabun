...
Parabun rewrite: `bun:simd.dot` for centroid norms, packed `Float32Array` data/centers, fused assignment loop with 8-way-unrolled inner dot, reformulated distance as `‖c‖² − 2·x·c`.
...
- **Why `bun:simd.matVec` isn't used.** The earlier draft called `matVec(dataMat, centers[k], N, D)` K times per iteration. Each call copies the full N × D matrix into the WASM scratch arena before dispatch — a 10 MB memcpy × 16 centroids × 4 iters = **640 MB of copies per run** for no algorithmic reason. An `out` parameter on matVec wouldn't help; the problem is the matrix copy on input. Either matVec needs a "matrix stays put across calls" mode, or k-means needs a dedicated `bun:simd` primitive (e.g. `assignToNearest(dataMat, centersMat, clusterID, centerNormsSq)`).
- **Where a real win lives.** Either (a) multi-threaded assignment via `bun:parallel.pmap` with `SharedArrayBuffer`-backed data (blocked by pmap's current per-call item cloning), or (b) GPU matmul via `bun:gpu.matmul` to compute the full N × K cross matrix in one dispatch. Both are out of scope for a "swap in a replacement" rewrite.
