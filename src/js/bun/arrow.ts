  ...
  get(i: number): ColumnGetResult {
    ...
    switch (this.type.kind) {
      ...
      case "struct": {
        ...
        if (!cs) throw new Error("@lyku/para-arrow Column.get: struct column has no children");
      ...
      case "map": {
        ...
        if (!this.child || !this.child.children || this.child.children.length !== 2) {
          throw new Error("@lyku/para-arrow Column.get: map column missing key/value children");
        }
        const offsets = this.values as Int32Array;
        const start = offsets[i];
        const end = offsets[i + 1];
...
function parseDecimalString(s: string, scale: number, columnName: string): bigint {
  ...
  if (!/^-?\d+(?:\.\d+)?(?:[eE][-+]?\d+)?$/.test(trimmed)) {
    throw new TypeError(
      `@lyku/para-arrow.fromRows: decimal128 string ${JSON.stringify(s)} not a valid decimal in column ${JSON.stringify(columnName)}`,
    );
  }
  ...
  if (!Number.isFinite(n)) {
    throw new TypeError(`@lyku/para-arrow.fromRows: decimal128 string ${JSON.stringify(s)} produced non-finite`);
  }
    ...
    switch (kind) {
      ...
      case "date32": {
        ...
        for (let i = 0; i < n; i++) {
          ...
          if (raw == null) {
            ...
            if (isNaN(d.getTime())) {
              throw new TypeError(
                `@lyku/para-arrow.fromRows: cannot coerce ${JSON.stringify(raw)} to date32 in column ${JSON.stringify(name)}`,
              );
            }
            arr[i] = Math.floor(d.getTime() / 86400000);
            v[i >> 3] |= 1 << (i & 7);
      ...
      case "decimal128": {
        ...
        if (t.kind !== "decimal128") {
          throw new TypeError(
            `@lyku/para-arrow.fromRows: decimal128 column ${JSON.stringify(name)} missing precision/scale on type`,
          );
        }
        if (!Number.isInteger(t.precision) || t.precision <= 0 || t.precision > 18) {
          throw new RangeError(
            `@lyku/para-arrow.fromRows: decimal128 precision must be 1..18 (FIXED_LEN_BYTE_ARRAY pending for higher); got ${t.precision} on column ${JSON.stringify(name)}`,
          );
        }
        if (!Number.isInteger(t.scale) || t.scale < 0 || t.scale > t.precision) {
          throw new RangeError(
            `@lyku/para-arrow.fromRows: decimal128 scale must be 0..precision (${t.precision}); got ${t.scale} on column ${JSON.stringify(name)}`,
          );
        }
        ...
        for (let i = 0; i < n; i++) {
          ...
          if (raw == null) {
            ...
            throw new TypeError(
              `@lyku/para-arrow.fromRows: cannot coerce ${JSON.stringify(raw)} to decimal128 in column ${JSON.stringify(name)}`,
            );
          }
        }
        values = arr;
      ...
      case "timestamp_micros": {
        ...
        for (let i = 0; i < n; i++) {
          ...
          if (raw == null) {
            ...
            if (isNaN(d.getTime())) {
              throw new TypeError(
                `@lyku/para-arrow.fromRows: cannot coerce ${JSON.stringify(raw)} to ${kind} in column ${JSON.stringify(name)}`,
              );
            }
...
function readBloomFilters(bytes: Uint8Array): Array<Map<string, { mightContain(v: any): boolean; numBytes: number }>> {
  if (!(bytes instanceof Uint8Array)) {
    throw new TypeError("@lyku/para-arrow.readBloomFilters: bytes must be a Uint8Array");
  }
