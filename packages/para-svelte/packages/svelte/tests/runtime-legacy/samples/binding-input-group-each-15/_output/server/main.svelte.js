...
export default function Main($$renderer, $$props) {
	$$renderer.component(($$renderer) => {
		...
		for (let $$index = 0, $$length = each_array.length; $$index < $$length; $$index++) {
			...
			$$renderer.push(`<input type="checkbox"${$.attr('checked', $.store_get($$store_subs ??= {}, '$order', order).iceCream[index].flavours.includes(flavour), true)} name="flavours"${$.attr('value', flavour)}/> ${$.escape(flavour)}`);
		}

		$$renderer.push(`<!--]--></form> <div id="output">${$.escape($.store_get($$store_subs ??= {}, '$order', order).iceCream[index].flavours.join('+'))}</div>`);
