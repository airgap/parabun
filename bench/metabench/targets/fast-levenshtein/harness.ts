// Four-way harness for fast-levenshtein.
...
const UPSTREAM = "./run-upstream.cjs";
...
const parabunRows = run([PARABUN, "run", UPSTREAM]);
console.log("Running rewrite on parabun…");
const rewriteRows = run([PARABUN, "run", REWRITE]);

const groups = [
  { label: "node (upstream)", rows: nodeRows },
  { label: "bun (upstream)", rows: bunRows },
  { label: "parabun drop-in", rows: parabunRows },
  { label: "parabun rewrite (pmap)", rows: rewriteRows },
];
...
console.log("\nnumerical agreement (value returned):");
for (const op of ops) {
...
console.log("\ntimings (ms) — min / med / max, best-of-7:");
const header = ["op".padEnd(18), ...groups.map(g => g.label.padEnd(26))];
console.log(header.join("  "));
for (const op of ops) {
  const row = [op.padEnd(18)];
  for (const g of groups) {
    const r = lookup(g, op);
    row.push(r ? `${fmt(r.min)} / ${fmt(r.med)} / ${fmt(r.max)}`.padEnd(26) : "—".padEnd(26));
  }
