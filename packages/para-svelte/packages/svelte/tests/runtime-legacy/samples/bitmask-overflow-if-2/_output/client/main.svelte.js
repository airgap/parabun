...
var root_2 = $.from_html(`if: false <div></div>`, 1);
...
export default function Main($$anchor, $$props) {
...

	{
		...
		var d = $.derived(() => (
			...
			$.deep_read_state(_40()),
			$.deep_read_state(_39()),
			$.untrack(() => _a().indexOf(_40()) > -1 && _40() === '40' && _39() === '39')
		));
	...
	$.template_effect(
		($0) => $.set_text(text, `${_0() ?? ''}${_1() ?? ''}${_2() ?? ''}${_3() ?? ''}${_4() ?? ''}${_5() ?? ''}${_6() ?? ''}${_7() ?? ''}${_8() ?? ''}${_9() ?? ''}${_10() ?? ''}${_11() ?? ''}${_12() ?? ''}${_13() ?? ''}${_14() ?? ''}${_15() ?? ''}${_16() ?? ''}${_17() ?? ''}${_18() ?? ''}${_19() ?? ''}${_20() ?? ''}${_21() ?? ''}${_22() ?? ''}${_23() ?? ''}${_24() ?? ''}${_25() ?? ''}${_26() ?? ''}${_27() ?? ''}${_28() ?? ''}${_29() ?? ''}${_30() ?? ''}${_31() ?? ''}${_32() ?? ''}${_33() ?? ''}${_34() ?? ''}${_35() ?? ''}${_36() ?? ''}${_37() ?? ''}${_38() ?? ''}${_39() ?? ''}${_40() ?? ''}
