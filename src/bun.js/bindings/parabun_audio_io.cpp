// Parabun: native audio I/O bindings for `bun:audio` capture / playback.
//
...
#if !defined(__linux__)
    throwTypeError(globalObject, scope, "bun:audio capture not yet implemented on this platform"_s);
    return {};
#else
...
#if !defined(__linux__)
    throwTypeError(globalObject, scope, "bun:audio playback not yet implemented on this platform"_s);
    return {};
#else
    if (callFrame->argumentCount() < 5) {
        throwTypeError(globalObject, scope,
