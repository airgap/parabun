...
What v1 ships (per `PLAN-bun-assistant.md` build order): `assistant.create`, `bot.run` / `turns` / `ask` / `say`, the four signals, in-memory transcript, sqlite-backed persistent memory, tool dispatch + MCP, barge-in, and wake word. Deferred follow-ups (tracked under LYK-760): RAG, scheduled prompts, and vision (VLM) turns.
