...
What v1 ships (per `PLAN-bun-assistant.md` build order): `assistant.create`, `bot.run` / `turns` / `ask` / `say`, the four signals, in-memory transcript, sqlite-backed persistent memory, tool dispatch + MCP, and barge-in. Deferred follow-ups (tracked under LYK-760): wake word, RAG, scheduled prompts, and vision (VLM) turns.
...
| shipped     | `bun:assistant` (Tier 2) | Three-line voice-assistant facade composing `bun:audio` + `bun:speech` + `bun:llm`. `bot.run` / `turns` / `ask` / `say` + reactive `state` / `history` / `lastTurn` / `interrupted` signals + sqlite-backed persistent memory + tool dispatch (inline + MCP) + VAD-driven barge-in (`bot.interrupt()`). Wake word, RAG, vision deferred to follow-ups. |
