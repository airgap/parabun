...
export function transformBindings(src) {
...
function transformOp(src, op, wrap) {
    if (!src.includes(op))
        return src;
...
function findNextTopLevelOp(src, from, op, inCode, findSpan) {
    let depth = 0;
    let i = from;
    while (i < src.length) {
        if (!inCode(i)) {
...
function scanBindLhsStart(src, opPos, inCode, findSpan) {
    let depth = 0;
    let i = opPos - 1;
...
function scanBindRhsEnd(src, startPos, inCode, findSpan) {
    let depth = 0;
    let i = startPos;
    while (i < src.length) {
        if (!inCode(i)) {
