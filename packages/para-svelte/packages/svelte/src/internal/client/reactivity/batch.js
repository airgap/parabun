...
import { active_effect, active_reaction, get, increment_write_version, is_dirty, update_effect } from "../runtime.js";
import * as e from "../errors.js";
import { flush_tasks, queue_micro_task } from "../dom/task.js";
import { DEV } from "esm-env";
import { invoke_error_boundary } from "../error-handling.js";
import { flush_eager_effects, mirror_to_para, old_values, set_eager_effects, source, update } from "./sources.js";
...
export class Batch {
  ...
  capture(source, value, is_derived = false) {
    ...
    if (!this.is_fork) {
      source.v = value;
      // Para Svelte bridge: chokepoint for set() + update_derived() writes.
      // Fork-mode skipped — fork commits flush via the loop in #commit() below
      // (which also mirrors). Error values skipped — they'd corrupt para
      // observers; consumers see the last good value until the error clears.
      if ((source.f & ERROR_VALUE) === 0) mirror_to_para(source, value);
    }
  }
    ...
    for (let batch = first_batch; batch !== null; batch = batch.#next) {
      ...
      if (others.length === 0) {
        ...
        var current_unequal = [...batch.current.keys()].filter(c =>
          this.current.has(c) ? /** @type {[any, boolean]} */ (this.current.get(c))[0] !== c.v : true,
        );
  ...
  return {
    commit: async () => {
      ...
      for (var [source, [value]] of batch.current) {
        ...
        // Para Svelte bridge: time-travel fork commit. The captured writes
        // were withheld from para observers while the fork was open
        // (Batch.capture skips mirror_to_para when this.is_fork). Flush them now.
        if ((source.f & ERROR_VALUE) === 0) mirror_to_para(source, value);
      }
