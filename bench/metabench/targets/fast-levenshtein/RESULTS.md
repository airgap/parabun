...
Parabun rewrite: same Myers kernel inlined into a `@lyku/para-parallel.pmap` worker, query string embedded into the worker function source so each item is a single string (not a `[query, candidate]` pair).
