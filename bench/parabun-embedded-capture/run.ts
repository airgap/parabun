// bun:camera + bun:audio compose demo
//
// Captures 2 seconds of video frames + microphone audio from the first
// available device on each side, saves:
//   - frame_<N>.jpg — first / middle / last frame as JPEG via bun:image
//   - audio.wav     — full capture as WAV via bun:audio.writeWav
//
// Run with:
...
console.log("=== bun:camera + bun:audio compose demo ===");
...
//    the bun:image round-trip path on the middle frame: decode → re-encode
...
if (frames.length === 0) {
  ...
  // Round-trip the middle frame through bun:image to prove the codec stack composes.
  ...
  console.log(`  wrote frame_mid_q85.jpg: ${reEncoded.byteLength} bytes (re-encoded via bun:image)`);
}
