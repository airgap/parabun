// Hardcoded module "@lyku/para-parallel"
//
...
function pool(opts: { size?: number; module: string }): Pool {
  ...
  if (typeof moduleArg !== "string" || moduleArg.length === 0) {
    throw new TypeError("@lyku/para-parallel pool: `module` must be an absolute path or file: URL string");
  }
  ...
  if (moduleArg.startsWith("./") || moduleArg.startsWith("../")) {
    throw new TypeError(
      "@lyku/para-parallel pool: `module` must be absolute — relative paths can't resolve in a worker. " +
  ...
  function run<T>(fnName: string, ...args: unknown[]): Promise<T> {
    if (disposed) return Promise.reject(new Error("@lyku/para-parallel pool: disposed"));
    // If every worker failed init, fail fast — queueing forever is worse.
    if (workers.length > 0 && workers.every(e => e.initOk === false)) {
      return Promise.reject(new Error(initFailureMessage ?? "@lyku/para-parallel pool: all workers failed to init"));
    }
  ...
  function dispose(): void {
    ...
    for (const [, p] of pending) p.reject(new Error("@lyku/para-parallel pool: disposed"));
    pending.clear();
    // Reject queued run calls so the user's awaiting promises terminate.
    for (const c of queue) c.reject(new Error("@lyku/para-parallel pool: disposed"));
    queue.length = 0;
  ...
  async function map<T, U>(fnName: string, array: readonly T[], opts?: { chunks?: number }): Promise<U[]> {
    if (typeof fnName !== "string" || fnName.length === 0) {
      throw new TypeError("@lyku/para-parallel pool.map: fnName must be a non-empty string");
    }
    if (!Array.isArray(array)) {
      throw new TypeError("@lyku/para-parallel pool.map: array must be a JS array");
    }
    const len = array.length;
    if (len === 0) return [];
  ...
  async function reduce<T, A>(
    ...
    if (typeof chunkFn !== "string" || chunkFn.length === 0) {
      throw new TypeError("@lyku/para-parallel pool.reduce: chunkFn must be a non-empty string");
    }
    if (typeof mergeFn !== "string" || mergeFn.length === 0) {
      throw new TypeError("@lyku/para-parallel pool.reduce: mergeFn must be a non-empty string");
    }
    if (!Array.isArray(array)) {
      throw new TypeError("@lyku/para-parallel pool.reduce: array must be a JS array");
    }
    const len = array.length;
...
class Mutex {
  ...
  constructor(sab?: SharedArrayBuffer) {
    if (sab !== undefined) {
      if (sab.byteLength < 4) throw new RangeError("@lyku/para-parallel: Mutex SAB must be >= 4 bytes");
      this.sab = sab;
    } else {
...
class Semaphore {
  ...
  constructor(initialPermits: number, sab?: SharedArrayBuffer) {
    ...
    if (sab !== undefined) {
      if (sab.byteLength < 4) throw new RangeError("@lyku/para-parallel: Semaphore SAB must be >= 4 bytes");
      this.sab = sab;
    } else {
      this.sab = new SharedArrayBuffer(4);
