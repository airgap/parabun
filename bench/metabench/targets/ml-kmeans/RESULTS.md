...
- **Where a real win lives.** Either (a) multi-threaded assignment via `para:parallel.pmap` with `SharedArrayBuffer`-backed data (blocked by pmap's current per-call item cloning), or (b) GPU matmul via `para:gpu.matmul` to compute the full N × K cross matrix in one dispatch. Both are out of scope for a "swap in a replacement" rewrite.
