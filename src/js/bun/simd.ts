...
function alloc(length: number, type: "f32" | "f64"): FArray {
  if (!isWasmAvailable()) throw new Error("@lyku/para-simd alloc requires the WASM backend");
  ...
  if (nextTop > ALLOC_COMMIT_BYTES) {
    throw new RangeError(
      `@lyku/para-simd alloc pool exhausted: requested ${byteLen} bytes at offset ${base}, pool ends at ${ALLOC_COMMIT_BYTES}`,
    );
  }
  allocTop = nextTop;
...
//
// Other bun: modules (notably @lyku/para-pipeline) need to know whether dispatching
