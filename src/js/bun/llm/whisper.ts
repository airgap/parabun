// Whisper-class STT for parabun:llm.
//
...
// inside `parabun:llm` and can't pull in `parabun:audio`'s mel implementation
...
// parabun:gpu is a sibling top-level builtin; the bundler resolves it through
...
function readBinModel(bytes: Uint8Array): BinModel {
  ...
  const magic = u32();
  if (magic !== MAGIC_GGML) {
    throw new Error(`parabun:llm whisper: not a whisper.cpp .bin file (magic 0x${magic.toString(16)} != 0x67676d6c)`);
  }
  ...
  while (cursor < bytes.byteLength) {
    ...
    if (nDims < 1 || nDims > 4 || nameLen <= 0 || nameLen > 1024) {
      throw new Error(`parabun:llm whisper: invalid tensor header at offset ${cursor - 12}`);
    }
    const dims: number[] = [];
    let nElements = 1;
    ...
    if (tFtype === 0) {
      ...
      if (nElements % QK !== 0) {
        throw new Error(
          `parabun:llm whisper: tensor "${name}" has ${nElements} elements, not a multiple of ${QK} (quantized block size)`,
        );
      }
      ...
      if (cursor + need > bytes.byteLength) {
        throw new Error(`parabun:llm whisper: tensor "${name}" runs past EOF`);
      }
      ...
      throw new Error(
        `parabun:llm whisper: unsupported tensor ftype=${tFtype} for "${name}" (F32, F16, Q4_0, Q5_0, Q5_1, Q8_0 supported)`,
      );
    }
...
function quantBlockSize(ftype: number): number {
  ...
  throw new Error(`parabun:llm whisper: no block size for ftype=${ftype}`);
}
...
function quantMatVec(w: QuantTensor, vec: Float32Array): Float32Array {
  if (vec.length !== w.cols) {
    throw new Error(`parabun:llm whisper: quantMatVec dim mismatch — vec ${vec.length} vs cols ${w.cols}`);
  }
  ...
  switch (w.ftype) {
    ...
    case 8:
      ...
      break;
    default:
      throw new Error(`parabun:llm whisper: quantMatVec ftype=${w.ftype} not supported`);
  }
  return dst;
}
...
// ─── FFT + mel preprocessing (inlined from parabun:audio) ─────────────────────
...
function fftInPlace(io: Float32Array, forward: boolean): void {
  const N = io.length >>> 1;
  if (N === 0 || (N & (N - 1)) !== 0) {
    throw new Error("parabun:llm whisper FFT: complex length must be a power of 2 ≥ 2");
  }
...
class WhisperTokenizer {
  ...
  prefix(language: string = "en"): number[] {
    ...
    if (lang === undefined) {
      throw new Error(`parabun:llm whisper: unknown language code "${language}"`);
    }
...
// GpuFloat32Array wrapper from parabun:gpu — same shape as a Float32Array
...
class WhisperModel {
  ...
  encode(mel: Float32Array, T: number): Float32Array {
    ...
    const Tenc = c2.Tout;
    if (Tenc > h.nAudioCtx) {
      throw new Error(`parabun:llm whisper: encoded ${Tenc} frames > audio ctx ${h.nAudioCtx}`);
    }
  ...
  detectLanguage(mel: Float32Array, T: number): { language: string; prob: number } {
    if (this.tokenizer.englishOnly) {
      throw new Error("parabun:llm whisper: detectLanguage requires a multilingual model");
    }
    const h = this.hparams;
  ...
  prepareState(state: WhisperState, encoderOut: Float32Array): void {
    ...
    if (state.encoderT !== encT || state.dim !== dim) {
      throw new Error(
        `parabun:llm whisper: state shape mismatch — state expects [${state.encoderT}, ${state.dim}], model has [${encT}, ${dim}]`,
      );
    }
  ...
  decodeStep(token: number, state: WhisperState): Float32Array {
    ...
    if (pos >= state.maxLen) {
      throw new Error(`parabun:llm whisper: decode position ${pos} exceeds nTextCtx=${state.maxLen}`);
    }
   ...
   *
   * Use `audio.melSpectrogram(audio, { mode: "whisper" })` from `parabun:audio`
  ...
  #transcribeMelInner(
    ...
    if (!Number.isInteger(beamSize) || beamSize < 1 || beamSize > 16) {
      throw new RangeError(`parabun:llm whisper: beamSize must be an integer in [1, 16], got ${beamSize}`);
    }
...
function mapWeights(bin: BinModel): WhisperWeights {
  const get = (name: string): Float32Array => {
    ...
    if (!t) throw new Error(`parabun:llm whisper: missing tensor "${name}"`);
    return t.data;
  };
...
// attn@V matmuls to parabun:gpu. Used by the encoder where N=1500 makes the
