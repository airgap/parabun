...
single-threaded vs `@lyku/para-parallel.pmap` across 8 workers. No SIMD — this
...
What makes the `@lyku/para-parallel` primitive feel light here:
...
- `variant-parabun.pjs` — same math, split across 8 workers via
  `@lyku/para-parallel.pmap` with independent PRNG streams.
