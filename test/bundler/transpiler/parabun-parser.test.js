...
describe("Parabun Parser", () => {
  ...
  describe("..= (await assignment)", () => {
    it("desugars to = await in async function", () => {
      const out = transpiler.transformSync(
        "async function f() { let x; x ..= fetch('/'); }"
      );
    ...
    it("works in const declaration", () => {
      const out = transpiler.transformSync(
        "async function f() { const x ..= fetch('/'); }"
      );
      expect(out).toContain("await");
      expect(out).toContain("fetch");
    });
    ...
    it("works at top level", () => {
      const out = transpiler.transformSync("const x ..= fetch('/');");
      expect(out).toContain("await");
      expect(out).toContain("fetch");
    });
  });
  ...
  describe("..! (catch operator)", () => {
    ...
    it("chains with ..&", () => {
      const out = transpiler.transformSync(
        "const x = promise ..! handler ..& cleanup;"
      );
      expect(out).toContain(".catch(handler)");
  ...
  describe("operator combinations", () => {
    it("..= with ..! and ..&", () => {
      const out = transpiler.transformSync(
        "async function f() { const x ..= fetch('/') ..! handler ..& cleanup; }"
      );
      expect(out).toContain("await");
      expect(out).toContain(".catch(handler)");
      expect(out).toContain(".finally(cleanup)");
    ...
    it("|> binds tighter than ..!", () => {
      ...
      const out = transpiler.transformSync(
        "const x = data |> transform ..! handler;"
      );
    ...
    it("|> binds tighter than ..&", () => {
      const out = transpiler.transformSync(
        "const x = data |> transform ..& cleanup;"
      );
      expect(out).toContain("transform(data)");
      expect(out).toContain(".finally(cleanup)");
    });
    ...
    it("full chain: |> then ..! then ..&", () => {
      const out = transpiler.transformSync(
        "const x = data |> process ..! handler ..& cleanup;"
      );
