...
| partial     | `bun:arrow` (Tier 2)  | In-memory columnar tables (`RecordBatch`, `Table`, `Column`), type inference from typed arrays, validity bitmaps, computes (`sum` / `mean` / `min` / `max` / `count` / `variance` / `stddev` / `quantile` / `median` / `distinct` / `filter` / `groupBy`), and Arrow IPC streaming format (`fromIPC` / `toIPC`, hand-rolled FlatBuffers — round-trips int32 / int64 / float32 / float64 / bool / utf8 with validity bitmaps). Parquet pending. |
