...
export default function Main($$anchor, $$props) {
	...
	$.await(node_1, promise2, null, void 0, ($$anchor, $$source) => {
		var $$value = $.derived_safe_equal(() => {
			var { [`leng${th}`]: l, [`wid${th}`]: w, height: h } = $.get($$source);

			return { l, w, h };
		});

		var l = $.derived_safe_equal(() => $.get($$value).l);
		var w = $.derived_safe_equal(() => $.get($$value).w);
		var h = $.derived_safe_equal(() => $.get($$value).h);
		...
		const computed_const_3 = $.derived_safe_equal(() => {
			const {
				...
				$.deep_read_state($.get(l)),
				$.deep_read_state($.get(w)),
				$.deep_read_state($.get(h)),
				$.untrack(() => calculate($.get(l), $.get(w), $.get(h)))
			);
