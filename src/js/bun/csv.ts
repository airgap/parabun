...
async function* tokenize(source: CsvSource, delimiter: string, quote: string): AsyncIterable<string[]> {
  if (delimiter.length !== 1) throw new TypeError("@lyku/para-csv: delimiter must be a single character");
  if (quote.length !== 1) throw new TypeError("@lyku/para-csv: quote must be a single character");
  const D = delimiter;
  const Q = quote;
