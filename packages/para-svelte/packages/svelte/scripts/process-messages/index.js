...
// Para Svelte: documentation/ was pruned from the fork; generated docs go to a
// scratch dir inside the package instead. Build still passes; the docs aren't
// consumed by anything we ship.
const DIR = "./.generated-docs";
...
function run() {
  ...
  for (const category of fs.readdirSync("messages")) {
    ...
    for (const file of fs.readdirSync(`messages/${category}`)) {
      ...
      const markdown = fs.readFileSync(`messages/${category}/${file}`, "utf-8").replace(/\r\n/g, "\n");
      ...
      fs.writeFileSync(`messages/${category}/${file}`, sorted.map(x => x._.trim()).join("\n\n") + "\n");
    }
  ...
  function transform(name, dest) {
    const source = fs.readFileSync(new URL(`./templates/${name}.js`, import.meta.url), "utf-8").replace(/\r\n/g, "\n");
            ...
            if (line.includes("PARAMETER")) {
              return vars
                .map((name, i) => {
                  ...
                  return optional ? ` * @param {string | undefined | null} [${name}]` : ` * @param {string} ${name}`;
                })
                .join("\n");
            }
