...
import { Batch, batch_values, eager_block_effects, schedule_effect, legacy_updates } from "./batch.js";
...
/**
 * Para Svelte: expose a Source's backing para signal. External para code can
 * read or subscribe to it; reading via .get() during a para effect tracks
 * the dependency, so a para effect can observe a Svelte source.
 *
 * Returns undefined for inputs that aren't Sources allocated by this fork
 * (defensive — shouldn't happen in practice, but keeps consumer code simple).
 *
 * @template V
