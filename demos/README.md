...
| `iot-waterer` ([.pts](iot-waterer.pts) / [.ts](iot-waterer.ts)) | Multi-plant auto waterer: per-plant moisture signal → dry-threshold → pump pulse → cooldown FSM, plus a tank-level sensor on a 4th ADC channel that pauses all watering when empty and notifies on the empty / refilled / low-water transitions via `when` (the .pts version uses `when tankEmpty start { … }` + `when stop { … }` paired form; the .ts version uses `signals.whenStart` directly). 3 example plants. `--simulate` works on any host; `--i2c <bus>` uses real ADS1115; `--demo-empty` drains the simulated tank fast so the alert cycle fires in seconds. | Optional. Real mode: I²C ADS1115 @ 0x48 (channels 0..2 = moisture, ch 3 = tank level) + GPIO relay per plant (BCM17 / 27 / 22). |
