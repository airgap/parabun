...
// dot / matmul still fall back to @lyku/para-simd.
...
// loses 3–10× to @lyku/para-simd at every size we care about because the
...
// doesn't win against @lyku/para-simd's tiled JS loop because per-call HtoD+sync
...
// dot: cold GPU loses 9–18× to @lyku/para-simd at every size we measured on an RTX
...
// varies by hardware; matVec / matmul / dot all lose to @lyku/para-simd at every
...
// kernel vs @lyku/para-simd, finds the smallest N where GPU wins by ≥10% (margin
...
// those just wrap the view and matVec falls through to @lyku/para-simd.
//
