...
// Parabun `@lyku/para-signals` — fine-grained reactive primitives.
...
describe("@lyku/para-signals", () => {
  ...
  it("signal: set to same value is a no-op (no notify)", async () => {
    const { stdout, exitCode } = await runFixture(
      "signals-noop",
      `
        import { signal, effect } from "@lyku/para-signals";
        const x = signal(1);
        let runs = 0;
        effect(() => { x.get(); runs++; });
        x.set(1); // same value
  ...
  it("batch: groups writes so effect fires once", async () => {
    const { stdout, exitCode } = await runFixture(
      "signals-batch",
      `
        import { signal, effect, batch } from "@lyku/para-signals";
        const a = signal(1);
        const b = signal(2);
        let runs = 0;
  ...
  it("untrack: reads without subscribing", async () => {
    const { stdout, exitCode } = await runFixture(
      "signals-untrack",
      `
        import { signal, effect, untrack } from "@lyku/para-signals";
        const a = signal(1);
        const b = signal(10);
        let runs = 0;
        effect(() => {
...
// Language-level sugar: `effect { body }` desugars to `require("@lyku/para-signals").effect(() => { body })`.
...
// `const NAME = require("@lyku/para-signals").signal(RHS)` at parse time, and NAME
...
describe("parabun: signal-bound assignment rewriting", () => {
  ...
  it("integration: batch coalesces multiple writes into one effect run", async () => {
    const { stdout, exitCode } = await runFixture(
      "sig-int-batch",
      `
        import { batch } from "@lyku/para-signals";
        signal a = 1;
        signal b = 2;
        let runs = 0;
        effect { a; b; runs++; }
  ...
  it("integration: untrack skips dep registration", async () => {
    const { stdout, exitCode } = await runFixture(
      "sig-int-untrack",
      `
        import { untrack } from "@lyku/para-signals";
        signal tracked = 1;
        signal silent = 0;
        let runs = 0;
        effect {
  ...
  it("integration: effect cleanup runs before each re-run and on dispose", async () => {
    const { stdout, exitCode } = await runFixture(
      "sig-int-cleanup",
      `
        import { effect as effectFn } from "@lyku/para-signals";
        signal v = 0;
        const log = [];
        const stop = effectFn(() => {
          const snapshot = v;
