...
This bench is what motivated the `@lyku/para-simd` reduce-op threshold, the
`@lyku/para-simd.matVec` bulk kernel, `@lyku/para-parallel`'s persistent worker
pool, `parabun:gpu`'s Tier-4 residency, and the `@lyku/para-simd.topK` selection
...
All rows except `baseline` use `@lyku/para-simd.topK` for selection; `baseline`
...
| simd-dot (per-row `@lyku/para-simd.dot`)       |  27.8 /  27.9 /  28.8     |      1.53×  |
| matvec (bulk `@lyku/para-simd.matVec`)         |  51.8 /  52.5 /  53.3     |      0.82×  |
...
6. **`gpu` crushes everything once top-K stops being the bottleneck.**
   ...
   `@lyku/para-simd.topK(scores, k)` is a scalar fixed-size-sorted-array
...
`@lyku/para-simd.matVec` behavior if CUDA/Metal isn't available — `parabun:gpu`
...
- `variant-simd.pjs` — per-row `@lyku/para-simd.dot` (one WASM call per row).
- `variant-matvec.pjs` — bulk `@lyku/para-simd.matVec` (one WASM call total).
...
- `variant-gpu.pjs` — `parabun:gpu.matVec` with the embedding matrix held on
  ...
  `@lyku/para-simd.matVec` otherwise.
