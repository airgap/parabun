...
|---|---|
| [`@lyku/para-parallel`](#parallel-execution-paraparallel) | Persistent worker pool with `pmap` / `preduce`, SAB-shared state via `@lyku/para-arena` |
| [`parabun:gpu`](#gpu-compute-paragpu) | Raw CUDA + Metal kernels in TypeScript template strings, NVRTC at runtime, fallback chain |
| [`@lyku/para-simd`](#simd-primitives-parasimd) | Typed-array SIMD primitives without WASM gymnastics |
| [`@lyku/para-pipeline`](#pipeline-fusion-parapipeline) | Typed-array pipeline fusion that promotes to GPU when inputs are large enough |
| [`@lyku/para-arena`](#buffer-pooling-paraarena) | Buffer pooling so worker boundaries don't cost a `Uint8Array` allocation per chunk |
...
| [`@lyku/para-mcp`](https://parabun.script.dev/docs/mcp/) | Model Context Protocol client — stdio + WebSocket transports. Composes structurally with `parabun:assistant`'s `tools:` option. |
...
- **Reactive surface**: `m.busy` (writable, refcounted across nested calls) and `m.device` (`"cuda"` / `"metal"` / `"cpu"`) are exposed as signals — pair with `@lyku/para-signals` to drive UI without polling.
...
`parabun:gpu` is a compute-only GPU surface (not graphics) that mirrors the hot parts of `@lyku/para-simd`. It probes a backend chain — Metal on darwin, CUDA on Linux/Windows, CPU fallback always available — and picks the first one whose runtime loads.
...
Two thresholds, not one: a **dispatch** threshold lets the GPU kernel run (so tests exercise the real path), and a **wins** threshold (`gpu.winsForSize(op, n, elemBytes)`) tells callers when routing through `parabun:gpu` actually beats `@lyku/para-simd`. Today `simdMap` wins at ≥ 1<<18 f32 elements; `matVec` is compiled and correct but not yet winning (the naive MSL kernel is bandwidth-bound on M1/M2).

`@lyku/para-pipeline`'s fusion tier reads `winsForSize` automatically — a fused affine chain over a large enough `Float32Array` promotes from stacked `simd.mulScalar`+`simd.addScalar` to `gpu.simdMap` without user code changes.
...
### Parallel Execution (`@lyku/para-parallel`)

`@lyku/para-parallel.pmap` spreads CPU-bound work across a persistent worker pool. The worker-safety contract is enforced at parse time via the `pure` keyword (see [Language Extensions](#language-extensions)) — no closures, no `this`, no module-level references, so `fn.toString()` round-trips cleanly into the worker context.
...
### SIMD Primitives (`@lyku/para-simd`)

`@lyku/para-simd` exposes WASM-backed `f32x4` and `f64x2` kernels for `Float32Array` and `Float64Array`:

```ts
import { dot, sum, mulScalar, matVec } from "@lyku/para-simd";
...
### Pipeline Fusion (`@lyku/para-pipeline`)

`@lyku/para-pipeline` is the runtime behind the `|>` operator (see [Language Extensions](#language-extensions)). Affine `map` chains over `Float32Array` / `Float64Array` compile down to a single SIMD pass, with no intermediate arrays and no per-element function calls.

```pts
import { map, sum } from "@lyku/para-pipeline";
...
Each `map` extends a `FusedChain` descriptor instead of wrapping another async generator. On a terminal (`sum`, `collect`, `toFloat32Array`, …), the runtime probes each map with three points: if the whole chain is affine it collapses to a single `(K, C)` and dispatches to `@lyku/para-simd` as one pass — `sum` becomes `K · simd.sum(data) + C · n`. Non-affine chains still fuse into a single `simd.simdMap(composed_fn, data)` call.

### Buffer Pooling (`@lyku/para-arena`)

`@lyku/para-arena` is a typed-array pool. If your hot path repeatedly allocates short-lived buffers of a known size — protocol decode scratch, per-request work buffers, ring stages — borrow from a `Pool` instead of calling `new Uint8Array(N)`:

```ts
import arena from "@lyku/para-arena";
...
parabun (@lyku/para-arena Pool)    248.8 ms      → 2.85×
```
...
- **Reactive surface**: `bot.state` (`"idle" | "listening" | "thinking" | "speaking"`), `bot.history` (the message array), `bot.lastTurn`, and `bot.interrupted` are all `@lyku/para-signals` Signals — wire them directly into a UI without polling.
...
### Streaming CSV (`@lyku/para-csv`)

`@lyku/para-csv.parseCsv(source, opts?)` is an async-generator CSV parser with full RFC 4180 quoting / escapes, configurable delimiter and quote character, header-mode (yields `Record<string, value>` rows), and per-cell type inference (`number` / `boolean` / `null`). An opt-in `parallel: true` mode chunks the input across `@lyku/para-parallel`'s worker pool when the input is large enough and contains no quoted cells; otherwise it falls through to the serial state machine.
...
### Fine-Grained Reactivity (`@lyku/para-signals`)

`@lyku/para-signals` is a reactive primitive — signals, computed derivations, and side-effects that re-run automatically when their reads change. Reads inside an `effect` or `derived` register a dep edge; writes invalidate downstream, and a microtask-scheduled flush re-runs only what observed a changed value.
...
| SQLite analytical post-processing (1 M rows × 8)        | **2.71×** on analytical (~10% end-to-end)   | `@lyku/para-simd` on columnar F64    |
...
| Streaming ETL (10 M Float32, 4-stage affine → fused)    | **50×** vs `.map` chain · **1.24×** vs tight loop | `@lyku/para-pipeline` fusion   |
...
- **Tier 0 — primitives** (shipped): `@lyku/para-simd`, `parabun:gpu`, `@lyku/para-parallel`, `@lyku/para-arena`, `@lyku/para-pipeline`, `@lyku/para-signals`, `@lyku/para-rtp`. These are the building blocks that reach hardware directly.
- **Tier 1 — composed** (shipped, plus `parabun:video` in progress): `parabun:image`, `parabun:audio`, `parabun:camera`, `@lyku/para-csv`, `parabun:llm`, `parabun:gpio`, `parabun:i2c`, `parabun:spi`. Codecs, capture devices, on-device LLM inference, and userspace peripheral access for SBCs — built on Tier 0.
- **Tier 2 — applications** (`parabun:speech` ships full STT + TTS, `parabun:assistant` ships the edge voice-assistant facade, `@lyku/para-arrow` ships the in-memory model + computes + IPC reader/writer with Parquet pending; `parabun:vision` ships orchestration with detector / OCR engines stubbed): application-shaped modules that compose Tier 1 into voice assistants, vision pipelines, and analytical queries. (HTTP serving lives inside `parabun:llm` as `llm.serve()`.)
...
| shipped     | `@lyku/para-csv`             | Streaming RFC 4180 parser with header / inference / quote handling. `parallel: true` is "off-the-main-thread" — see the inline disclaimer above. |
| shipped     | `@lyku/para-rtp`             | RFC 3550 packet pack/parse + jitter-buffer for the Opus path; transport for the codec stack.          |
...
| next        | `@lyku/para-parallel` v2     | Closure-aware persistent worker pool + `SharedArrayBuffer` channels. Lifts today's `pmap` ceiling.    |
| planned     | `parabun:image` AVIF      | AVIF decode + encode (libavif + AOM / dav1d vendor add). Rounds out the codec coverage matrix.        |

`parabun:llm` becomes the proof-of-concept for the stack — "we built llama inference using `parabun:gpu` + `@lyku/para-simd` + `@lyku/para-parallel`; you can build similar things with the same building blocks" — rather than the headline product. Parabun is positioned as a perf runtime, not an AI runtime.
...
Mark functions as `pure` to make purity visible and enforced. The parser rejects `this` access inside pure functions at compile time — which is what makes `@lyku/para-parallel.pmap` safe to ship `fn.toString()` into a worker.
...
Pair it with [`@lyku/para-pipeline`](#pipeline-fusion-parapipeline) for fused typed-array map chains.
...
Desugars to `require("@lyku/para-arena").scope(() => { ... })`. This is **latency-smoothing, not a bump allocator** — the heap still pays the eventual collection cost, just at a time of the caller's choosing.
...
| `arena { ... }` | statement-level | `require("@lyku/para-arena").scope(() => { ... })` |
| `effect { ... }` | statement-level | `require("@lyku/para-signals").effect(() => { ... })` |
| `signal x = v` | statement-level | `const x = require("@lyku/para-signals").signal(v)` (or `.derived(() => v)`) |
| `A ~> B` | assignment | `require("@lyku/para-signals").effect(() => { B = A; })` |
| `A ~> B when C` | assignment | `require("@lyku/para-signals").effect(() => { if (C) B = A; })` |
