...
(`pure function`, `|>` pipelines) and runtime modules (`@lyku/para-simd`,
`@lyku/para-parallel`, `@lyku/para-pipeline`) on real-world shaped workloads.
...
| [parabun-monte-carlo](./parabun-monte-carlo/README.md)               | `@lyku/para-parallel.pmap` alone (no SIMD/SAB)      |  5.56×  |
| [parabun-streaming-etl](./parabun-streaming-etl/README.md)           | `@lyku/para-pipeline` fusion (affine → SIMD)        | 50× vs `.map` chain (1.24× vs hand-rolled loop) |
...
- **monte-carlo** is the pure `@lyku/para-parallel.pmap` showcase. No SIMD, no SAB,
  just a `pure function` kernel chunked across 8 workers with independent
  PRNG streams seeded from a Weyl constant.
- **streaming-etl** is pure `@lyku/para-pipeline` fusion. A 4-stage affine chain
