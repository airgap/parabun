...
pub fn NewParser_(
    ...
    return struct {
        ...
        pub const parsePurePrefixExpr = parse_zig.parsePurePrefixExpr;
        pub const parsePureAsyncPrefixExpr = parse_zig.parsePureAsyncPrefixExpr;
        ...
        /// Parabun: refs declared by `signal NAME = RHS`. Visit pass rewrites
        /// bare reads of these refs into `.get()` calls and assignments into
        ...
        /// Used for auto-derive detection: if the RHS of a `signal NAME = RHS`
        /// references any name already in this set, the decl is promoted from
        /// `signal(RHS)` to `derived(() => RHS)`.
        ///
        ...
        /// of auto-derive detection — every `signal NAME = RHS` becomes a
        /// plain `signal(RHS)` regardless of RHS content.
