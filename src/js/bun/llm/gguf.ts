// GGUF v3 loader for Parabun's bun:llm module.
//
...
class GGUFFile {
  ...
  constructor(bytes: Uint8Array) {
    ...
    const magic = r.u32();
    if (magic !== 0x46554747) {
      throw new Error(`bun:llm: not a GGUF file (magic ${magic.toString(16)}, expected 46554747)`);
    }
    this.version = r.u32();
    if (this.version !== 3) {
      throw new Error(`bun:llm: unsupported GGUF version ${this.version} (only v3 is implemented)`);
    }
  ...
  tensorF32(name: string): Float32Array {
    ...
    switch (info.type) {
      ...
      case GGML_TYPE_Q8_0: {
        if (nElems % QK8_0 !== 0) {
          throw new Error(`bun:llm: Q8_0 tensor "${name}" element count ${nElems} not a multiple of ${QK8_0}`);
        }
      ...
      case GGML_TYPE_Q2_K: {
        if (nElems % QK_K !== 0) {
          throw new Error(`bun:llm: Q2_K tensor "${name}" element count ${nElems} not a multiple of ${QK_K}`);
        }
      ...
      case GGML_TYPE_Q3_K: {
        if (nElems % QK_K !== 0) {
          throw new Error(`bun:llm: Q3_K tensor "${name}" element count ${nElems} not a multiple of ${QK_K}`);
        }
      ...
      case GGML_TYPE_Q4_K: {
        if (nElems % QK_K !== 0) {
          throw new Error(`bun:llm: Q4_K tensor "${name}" element count ${nElems} not a multiple of ${QK_K}`);
        }
      ...
      case GGML_TYPE_Q5_K: {
        if (nElems % QK_K !== 0) {
          throw new Error(`bun:llm: Q5_K tensor "${name}" element count ${nElems} not a multiple of ${QK_K}`);
        }
      ...
      case GGML_TYPE_Q6_K: {
        if (nElems % QK_K !== 0) {
          throw new Error(`bun:llm: Q6_K tensor "${name}" element count ${nElems} not a multiple of ${QK_K}`);
        }
        out = dequantQ6K(this.#bytes, this.#bytes.byteOffset + absOffset, nElems);
        break;
      }
      default:
        throw new Error(`bun:llm: unsupported GGML type ${info.type} for tensor "${name}"`);
    }
  ...
  // Raw byte view of a tensor's data region — no dequant. Used by bun:llm's
  ...
  string(key: string): string {
    ...
    if (typeof v !== "string") {
      throw new Error(`bun:llm: metadata "${key}" is ${typeof v}, want string`);
    }
    return v;
  }
  number(key: string): number {
    ...
    throw new Error(`bun:llm: metadata "${key}" is ${typeof v}, want number`);
  }
  array(key: string): GGUFValue[] {
    const v = this.metadata.get(key);
    if (!Array.isArray(v)) {
      throw new Error(`bun:llm: metadata "${key}" is ${typeof v}, want array`);
    }
    return v;
  }
}
...
class Reader {
  ...
  #readValue(tag: number): GGUFValue {
    switch (tag) {
      ...
      default:
        throw new Error(`bun:llm: unknown GGUF value tag ${tag} at byte ${this.pos - 4}`);
    }
  }
}
...
function tensorByteLength(type: number, dims: number[]): number {
  ...
  switch (type) {
    ...
    default:
      throw new Error(`bun:llm: unsupported GGML type ${type} in tensor-info`);
  }
}
